...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      setLoading(true);
      try {
        const response = await axios.get('http://localhost:3000/api/v1/blog/bulk?filter', {
          headers: {
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/api/v1/Updateblog`,
        {
          userId,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/api/v1/deleteblog`, {
